...
Expression[] getExpressions(Expression e, Node p = null) {
	...
	if (auto be = cast(BinaryExpression) e) {
		...
		foreach(arg;ce.args) {
			result ~= arg ~ getExpressions(arg, e);
		}
		...
		result ~= e ~ getExpressions(ie.indexed, e) ~ getExpressions(ie.index,e);
	} else if (auto ve = cast(VariableExpression) e) {
		result ~= e ~ (ve.var.value ? getExpressions(ve.var.value, e) : []);
	} else if (auto ce = cast(CallExpression) e) {
		result ~= e ~ getExpressions(ce.callee, e);
		foreach(arg;ce.args) {
